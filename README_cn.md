...
的比特币 / 以太坊链上基本面。四个懒加载标签页，各自独立接口，共享 10 分钟
缓存：
...
| **地域需求与出价系数** | 所选词在全美 210 个 DMA（或某国各地区）的需求分布，并给出每个地域的 Google Ads 地理位置出价调整建议 |
...
- **建议出价系数 Suggested modifier** —
  `clamp((地域分数 / 平均分数 − 1) × 100, −90…+900)`，边界正是 Google Ads
  地理位置出价调整的实际取值范围。它映射的是*相对需求*而非转化数据 ——
  只能作为地理定向的起点，绝不能当成最终出价。
